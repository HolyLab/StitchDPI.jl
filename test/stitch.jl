...
ysz_chip = 1000
ysz_roi = 500
tfm0 = StitchDPI.initial_guess_tfm(ysz_chip, ysz_roi)
tfm_tol = 1e-6
...
#one pixel overlap
ft, rt, fb, rb = fake_split(8, 4; frac_overlap = 0.25, xsz = 4)
@test all(ft[:,3:6].==rt)
@test all(fb[:,3:6].==rb)
@test all(rt[:,4].==rb[:,1])

#stitching
ft, rt, fb, rb = fake_split(80, 40; frac_overlap = 0.0, xsz = 40)
...
ft, rt, fb, rb = fake_split(80, 40; frac_overlap = 0.25, xsz = 40)
tfmfound, mm = stitch_tfm(StitchDPI.flipy(fb), rt, StitchDPI.flipy(rb); maxevals=500)
@test sum(abs.(tfmfound.linear - LinearAlgebra.I(2))) < tfm_tol

...
function stitch_slicewise(img_top, img_bottom, stitch_tfm, slice_tfms, out_type=Float64; octr=center(view(img_top,:,:,1,1)), correct_bias=true, sqrt_tfm=false, flip_y_bottom=true)
    tfms_top, tfms_bottom = recenter_tfms(slice_tfms, stitch_tfm, img_top, octr)
...
function recenter_tfms(slice_tfms, stitch_tfm, cam1, octr)
    tfmstop = []
    tfmsbottom = []
    cam1ctr = center(view(cam1, :,:,1,1)).-octr
    ...
    for i =1:length(slice_tfms)
        ...
        push!(tfmsbottom, stitch_tfm ∘ recenter(slice_tfms[i], cam2ctr))
    end
